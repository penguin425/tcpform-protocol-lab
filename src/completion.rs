...
#[cfg(test)]
const COMMANDS: &str = "validate list plan visualize serve fmt migrate init template schema ci-snapshot ci-report lsp gate bundle replay-bundle anonymize orchestrate proxy explore generate-faults plugin tls-audit differential platform run test doctor completion help";
...
const BASH: &str = r#"_tcpform() {
  ...
  commands="validate list plan visualize serve fmt migrate init template schema ci-snapshot ci-report lsp gate bundle replay-bundle anonymize orchestrate proxy explore generate-faults plugin tls-audit differential platform run test doctor completion help"
  case "$prev" in
